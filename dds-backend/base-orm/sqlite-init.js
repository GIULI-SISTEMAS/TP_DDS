...
async function CrearBaseSiNoExiste() {
  ...
  // Verificamos si la tabla 'usuarios' existe
  res = await db.get(
    "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'usuarios'",
    []
  );
  if (res.contar > 0) existe = true; //Si contar > 0, la tabla ya existe
  ...
  if (!existe) {
    ...
    existe = false; // Reiniciamos la variable para la siguiente tabla

    // Verificamos si la tabla 'articulosfamilias' existe
    res = await db.get(
    ...
    existe = false;
    // Verificamos si la tabla 'articulos' existe
    sql =
